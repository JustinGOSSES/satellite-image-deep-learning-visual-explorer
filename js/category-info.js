app.controller('gitHubDataController', function($scope, $http, $window, Category) {
    var getCategoryInfo = $http.get('/satellite-image-deep-learning-visual-explorer/category/category_info.json', {
        cache: true
    });

    var getReposTopics = $http.get('/satellite-image-deep-learning-visual-explorer/explore/github-data/intRepos_Topics.json', {
        cache: true
    });

    var getReposInfo = $http.get('/satellite-image-deep-learning-visual-explorer/explore/github-data/intReposInfo.json', {
        cache: true
    });

    var getReposLogos = $http.get('/satellite-image-deep-learning-visual-explorer/assets/images/logos/repo_logos.json', {
        cache: true
    });
        ...
        getReposTopics.then(function(response) {
            ...
            getReposLogos.then(function(response) {
                ...
                $scope.categoryHref = function(nametag) {
                    ...
                    $window.location.href = '/satellite-image-deep-learning-visual-explorer/category#' + result;
                };
            });
        });
    });
